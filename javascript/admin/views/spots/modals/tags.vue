<template>
  <div class="modal fade" id="modalSpotTags" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
      <div class="modal-content !max-w-[800px]">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Tags perso</h4>
        </div>
          ...
          <table class="table table-stripped">
            ...
            <tbody>
              <tr v-for="tag in tags" :key="tag.id" style="height: 45px">
                <td>
                  <div class="tag-item">
                    <span @click="toggle($event, tag.id)" class="tagged" :class="{'fadded': !spot.device_tag_ids.includes(tag.id)}" :style="'background-color: ' + tag.color">{{ tag.name }}</span>
                  </div>
  ...
  methods: {
    ...
    create: function(e) {
      this.$store.dispatch("TagStore/create", {
        ...
        this.$store.dispatch("TagStore/index", this.spot.id);
